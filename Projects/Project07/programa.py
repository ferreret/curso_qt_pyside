...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def dibujarTabla(self):
        ...
        self.tableWidget.setHorizontalHeaderLabels(
            [str(h) for h in range(n_columnas)])
        # Rellenar cada celda en la tabla
        for indice, sonda in enumerate(self.sondas):
            for j, temperatura in enumerate(sonda["valores"]):
                self.tableWidget.setItem(indice, j,
                                         QtWidgets.QTableWidgetItem(str(temperatura)))
    ...
    def generarReporte(self):
        try:
            ...
            styler = df.style.applymap(
                lambda valor: 'color: red' if valor < 0 else 'color:black')
            ...
            msg = QtWidgets.QMessageBox.critical(
                self, "Ups", f"Error generando HTML \n\n{e}")
        else:
    ...
    def generarPDF(self):
        ...
        try:
            ...
            pdfkit.from_file(absPath("reporte.html"), absPath(
                "reporte.pdf"), options=options)
        except Exception as e:
            # Show critial error
            msg = QtWidgets.QMessageBox.critical(
                self, "Ups", f"Error generando PDF \n\n{e}")
        else:
